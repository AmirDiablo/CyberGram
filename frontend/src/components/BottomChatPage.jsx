...
const BottomChatPage = ({result, chatId, socketId}) => {
    ...
    const send = async(_value)=> {
        await socket.emit("sendMessage", {senderId, message, chatId})
            ...
            const response = await fetch("http://localhost:3000/api/message", {
                method: "POST",
                body: JSON.stringify({sender: senderId, content: message, reciever: recieverId}),
                headers: {
    ...
    const sendImage = async()=> {
        ...
        const response = await axios.post("http://localhost:3000/api/message/sendImage/?sender="+senderId+"&reciever="+recieverId, formData)
    ...
    const startRecording = async () => {
        ...
        mediaRecorderRef.current.onstop = async() => {
            ...
            const response = await axios.post("http://localhost:3000/api/message/voice/?sender="+senderId+"&reciever="+recieverId, formData)
            if(response.status === 200) {
                console.log(response.data)
                console.log("everything is ok")
    ...
    return ( 
        <div className="bottomChatPage" onClick={closeActions}>
            ...
            {/*{message != "" || audioURL ? <IoSend  className="send" onClick={()=> send(audioURL)}/> : <MdOutlineKeyboardVoice className="micIscon" onClick={openMicMenu} />}*/}        
        </div>
     );
}
 
export default BottomChatPage;